...
Additionally, we are interested in seeing if there is a significant difference between the total rainfall in the first half of the year and the second half. This leads us to create a categorical variable, which allows us to compare the first and last 6 months of each year.

Using the observations recorded each month, we will apply linear regression to create a model to predict the total rainfall within Paisley. Furthermore, we shall use numerical summaries and visual plots to compare the first and second halves of the year.
